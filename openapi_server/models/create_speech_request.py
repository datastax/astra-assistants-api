...
class CreateSpeechRequest(BaseModel):
    ...
    response_format: Optional[StrictStr] = Field(default='mp3', description="The format to return audio in.  Supported formats are `mp3`, `opus`, `aac`, `flac`, `pcm`, and `wav`.   The `pcm` audio format, similar to `wav` but without a header, utilizes a 24kHz sample rate, mono channel, and 16-bit depth in signed little-endian format.")
    ...
    def response_format_validate_enum(cls, value):
        ...
        if value not in ('mp3', 'opus', 'aac', 'flac', 'pcm', 'wav'):
            raise ValueError("must be one of enum values ('mp3', 'opus', 'aac', 'flac', 'pcm', 'wav')")
        return value
